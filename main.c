...
int main(int argc, char **argv) {
    ...
    // Maximum grid capacity in BITS for the grid itself: 64 KiB * 8
    const uint64_t CAPACITY_BITS = 64ULL * 1024ULL * 8ULL;
    uint64_t requested_bits = (uint64_t)width * (uint64_t)height;

    if (requested_bits > CAPACITY_BITS) {
        // Compute a proportional scale to keep aspect ratio
        double scale = sqrt((double)CAPACITY_BITS / (double)requested_bits);
        int new_width = (int)(width * scale);
        int new_height = (int)(height * scale);
        if (new_width < 1) new_width = 1;
        if (new_height < 1) new_height = 1;

        // Make sure product is <= CAPACITY_BITS (adjust conservatively)
        while ((uint64_t)new_width * (uint64_t)new_height > CAPACITY_BITS) {
        ...
        fprintf(stderr,
            "Requested grid %dx%d exceeds 64 KiB (%" PRIu64 " bits). "
            "Resizing to %dx%d to fit.\n",
            width, height, CAPACITY_BITS, new_width, new_height);

        width = new_width;
        height = new_height;
    }

    // Now try to create the world (world_create still checks underlying memory)
